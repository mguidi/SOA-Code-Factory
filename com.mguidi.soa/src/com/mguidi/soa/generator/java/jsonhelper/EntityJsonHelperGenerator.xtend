...
class EntityJsonHelperGenerator {
			...
			«FOR feature: features»
				...
				«ENDIF»
					if (reader.peek() == JsonToken.NULL) {
						...
						obj.«feature.featureSetterName»(«(feature.type as EntitiesFeature).type.qualifiedClassNameHelper».fromJson(reader));
						«ELSEIF (feature.type as EntitiesFeature).type instanceof com.mguidi.soa.soa.Enum»
						try {
							obj.«feature.featureSetterName»(«((feature.type as EntitiesFeature).type as com.mguidi.soa.soa.Enum).nextValue»);
						} catch (java.lang.IllegalArgumentException e) {
							obj.«feature.featureSetterName»(«(feature.type as EntitiesFeature).type.qualifiedClassName»._UNDEFINED_);
						}
						«ENDIF»
						«ELSEIF feature.type instanceof PrimitiveFeature»
							«IF (feature.type as PrimitiveFeature).type == PrimitiveType::DECIMAL»
							try {
								obj.«feature.featureSetterName»(«(feature.type as PrimitiveFeature).type.nextValue»);
						...
						«var index = (feature.type as GenericListFeature).depthToBaseType - i - 1»
							...
							«ELSE»
							obj.«feature.featureSetterName»(list«index»);
							«ENDIF»
							reader.beginArray();
							while (reader.hasNext()) {
						«ENDFOR»
	...
	def writeFeatures(String className, List<Feature> features) '''
		«FOR feature: features»
			if (obj.«feature.featureGetterName»() != null) {
				writer.name(«className».«feature.key»);
				«IF feature.type instanceof EntitiesFeature»
				«IF (feature.type as EntitiesFeature).type instanceof Entity»
				«(feature.type as EntitiesFeature).type.qualifiedClassNameHelper».writeJson(obj.«feature.featureGetterName»(), writer);
				«ELSEIF (feature.type as EntitiesFeature).type instanceof com.mguidi.soa.soa.Enum»
				«((feature.type as EntitiesFeature).type as com.mguidi.soa.soa.Enum).writeValue("obj."+feature.featureGetterName+"()")»
				«ENDIF»
				«ELSEIF feature.type instanceof PrimitiveFeature»
				«(feature.type as PrimitiveFeature).type.writeValue("obj."+feature.featureGetterName+"()")»;
				«ELSEIF feature.type instanceof GenericListFeature»
					«FOR i: 0 ..<(feature.type as GenericListFeature).depthToBaseType»
					«var index = (feature.type as GenericListFeature).depthToBaseType - i - 1»
						writer.beginArray();
						«IF i==0»
						for («(feature.type as GenericListFeature).declarationAtDepth(i+1)» list«index» : obj.«feature.featureGetterName»()) {
						«ELSE»
