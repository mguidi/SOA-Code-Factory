...
class InstallScriptGenerator {
	...
	def generateInstallScriptServletJson(List<Architecture> architectures) '''
		«FOR e: architectures»
		«IF e.module.services.size > 0»
		gradle -b «e.applicationId»/«e.moduleName»/«e.version»/servlet-json/build.gradle build uploadArchives
		«ENDIF»
		«ENDFOR»
	'''
